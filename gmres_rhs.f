c-----------------------------------------------------------------------
c
      subroutine gmres_rhs
      include 'parameter.inc'
      include 'surface.inc'
      include 'field.inc'
      integer isphr,nijk,nn,id,jd,kd,ic,jc,kc
      parameter(nijk=2,nn=3)
      real*8 xn,yn,zn,sn,xx,yy,zz,sx,sy,sz,foo,gacobi
      ...
      do m2=0,ns2-1
        do m1=0,ns1
          ...
          if(nn.eq.2) then
c            fq(m1,m2,m)=-(-1.0d0*pp(1)+1.0d0*pp(2))/sn
            fq(m1,m2,m)=2.0d0*pp(1)-1.0d0*pp(2)
          endif
          if(nn.eq.3) then
c            fq(m1,m2,m)=-(-2.5d0*pp(1)+4.0d0*pp(2)-1.5d0*pp(3))/sn
            fq(m1,m2,m)=3.0d0*pp(1)-3.0d0*pp(2)+1.0d0*pp(3)
          endif
